...
private let content = MainViewContent(
  tableContent: .init(
    tableHeader: ["#", "Worker", "Failed Count"],
    content: [["Worker 1", "hello"]]
  )
)

private let failedV = (title.contramap { _ in "Failed" }
  <> tableComponent
  <> footerView.contramap { _ in }).map(flip(curry(baseView))(.failed) >>> pure)
