...
private let dashBoardV: View<DashBoardData, [Node]> =
    (title.contramap { _ in "Redis Overview" }
    ...
    <> footerView.contramap { _ in }).map(
    flip(curry(baseView))(.overview) >>> pure
  )
...
private let title = View<String, [Node]> { content in
  return [br, h3([classAtr("h2")], [.text(content)])]
}
...
private let chartView = View<(), [Node]> {
  return [div(
    ...
    [
      ...
      div(
        [classAtr("btn-toolbar mb-2 mb-md-0")],
        [div(
          [classAtr("btn-group mr-2")],
          [
            button(
              [classAtr("btn btn-sm btn-outline-secondary")],
              [.text("Share")]
            ),
            button(
              [classAtr("btn btn-sm btn-outline-secondary")],
